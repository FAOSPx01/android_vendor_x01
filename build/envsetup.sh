function __print_x01_functions_help() {
cat <<EOF
...
function breakfast()
{
    target=$1
    local variant=$2
    local aosp_target_release=$(cat ${ANDROID_BUILD_TOP}/vendor/x01/vars/aosp_target_release 2>/dev/null)
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            lunch x01_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/x01-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.x01.device | grep -q "$X01_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $X01_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function lineageremote()
{
    ...
    local X01_USER=$(git config --get review.review.lineageos.org.username)
    if [ -z "$X01_USER" ]
    then
        git remote add lineage ssh://review.lineageos.org:29418/$PFX$PROJECT
    else
        git remote add lineage ssh://$X01_USER@review.lineageos.org:29418/$PFX$PROJECT
    fi
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.x01.device | grep -q "$X01_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.x01.device | grep -q "$X01_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/x01/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $X01_FIXUP_COMMON_OUT ]; then
