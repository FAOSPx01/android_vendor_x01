...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from LineageOS Github (http://github.com/FAOSPx01)." % device)
...
if not depsonly:
    githubreq = urllib.request.Request("https://raw.githubusercontent.com/FAOSPx01/mirror/main/default.xml")
    try:
...
def is_in_manifest(projectpath):
    ...
    try:
        lm = ElementTree.parse(".repo/manifests/snippets/x01.xml")
        lm = lm.getroot()
    except:
        lm = ElementTree.Element("manifest")
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/x01.dependencies'
    syncable_repos = []
    verify_repos = []
...
def get_default_or_fallback_revision(repo_name):
    ...
    githubreq = urllib.request.Request("https://api.github.com/repos/FAOSPx01/" + repo_name + "/branches")
    add_auth(githubreq)
...
print("Repository for %s not found in the FAOSPx01 Github repository list. If this is in error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
